...
let match = html.match(/function renderSearchProduct\(\) \{([\s\S]*?)main\.innerHTML = \`([\s\S]*?)<div class="grid grid-cols-2/);
if (match) {
    const bannerHtml = `
    ...
    html = html.replace(match[0], `function renderSearchProduct() {${match[1]}main.innerHTML = \`${newInner}`);
    fs.writeFileSync('index.html', html);
    console.log("Updated renderSearchProduct");
} else {
